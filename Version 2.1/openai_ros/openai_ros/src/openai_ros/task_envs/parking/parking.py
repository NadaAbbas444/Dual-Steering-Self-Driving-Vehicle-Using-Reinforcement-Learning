...
class ParkingTaskENV(parking_env.ParkingENV):
    def __init__(self):
        ...
        number_actions = rospy.get_param('/parking/n_actions')
    ...
    def _set_action(self, action):
        ...
        if action == 0:  # FORWARD
            ...
            steering_angle = -1*self.steering_angle
    ...
    def _get_obs(self):
        ...
        discretized_observations = self.discretize_observation(laser_scan, self.new_ranges)
    ...
    def discretize_observation(self, data, new_ranges):
        ...
        discretized_ranges = [round(min(min(data.ranges[0:59]), 15)*4)/4,
                        round(min(min(data.ranges[60:119]), 15)*4)/4,
                        round(min(min(data.ranges[120:179]), 15)*4)/4,
        ...
        for i, item in enumerate(data.ranges):
            if item == float('Inf') or numpy.isinf(item):
                ...
                discretized_ranges.append(round(item, self.dec_obs))
'''
        for item in discretized_ranges:
            if (self.min_range >= item > 0):
                rospy.logerr("done Validation >>> item=" +
                                str(item)+"< "+str(self.min_range))
                self._episode_done = True
